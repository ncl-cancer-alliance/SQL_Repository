-- This filters PCN level NCL fingertips data into a more usable format.
-- Contact: jake.kealey@nhs.net

CREATE OR REPLACE DYNAMIC TABLE DEV__MODELLING.CANCER__FINGERTIPS.INDICATOR_DATA_PCN(
    INDICATOR_ID,
    INDICATOR_NAME,
    PCN_CODE,
    PCN_NAME,
...
COMMENT = 'Dynamic table to format PCN Fingertips Data'
AS

SELECT
    cf."Indicator ID" AS INDICATOR_ID,
    cf."Indicator Name" AS INDICATOR_NAME,
    cf."Area Code" AS PCN_CODE,
    gp_ref.PCN_NAME,
...
ON cf."Area Code" = gp_ref.PCN_CODE

--Filter to PCN data
WHERE ma.AREA_ID = 204
--Filter to NCL data
