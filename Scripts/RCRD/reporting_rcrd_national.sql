...
create or replace dynamic table DEV__REPORTING.CANCER__RCRD.CANCER__RCRD__NATIONAL(
 ...
 COMMENT='Dynamic table to clean RCRD National data. \n\nContact: eric.pinto@nhs.net'
 as
...
WITH base AS (
    SELECT 
        ...
        CASE 
            WHEN CANCER_GROUP_BROAD = 'Blood cancer (haematological neoplasms)' THEN 'Haematological'
        ...
        CASE 
            WHEN CANCER_GROUP_DETAILED = 'Broad group total' 
                THEN CONCAT(
                    CASE 
                        WHEN CANCER_GROUP_BROAD = 'Blood cancer (haematological neoplasms)' THEN 'Haematological'
                        WHEN CANCER_GROUP_BROAD = 'Respiratory and intrathoracic' THEN 'Lung'
            ...
            WHEN CANCER_GROUP_DETAILED IS NULL THEN 'All Cancer Sites - Total'
            ELSE CANCER_GROUP_DETAILED
        END AS NEW_CANCER_GROUP_DETAILED,

        CASE 
            WHEN CANCER_GROUP_BROAD IN (
                'Breast','Cervix','Colon','Corpus Uteri','Lung','Melanoma',
                'Non-Hodgkin lymphoma','Ovary','Pancreas','Prostate','Rectal',
                'Stomach','Uterus','Bladder','Kidney','Hodgkin lymphoma',
                'Oesophagus','Bowel','Gynaecological','Oesophago-gastric',
                'Respiratory and intrathoracic','Blood cancer (haematological neoplasms)',
                'Urological excl. prostate','Upper GI excl. OG'
            ) THEN TRUE
            ELSE FALSE
        END AS IS_STAGEABLE,
